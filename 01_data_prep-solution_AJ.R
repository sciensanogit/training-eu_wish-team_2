...
# load data ----
base_wfs <- "https://data.geo.be/ws/sciensano/wfs?SERVICE=WFS&REQUEST=GetFeature&VERSION=2.0.0&outputFormat=csv&TYPENAMES="

# SARS-CoV-2 data (WFS radi)
df_sc <- read.csv(paste0(base_wfs, "sciensano:wastewatertreatmentplantscovid"))

# PMMV data (WFS radi)
df_pmmv <- read.csv(paste0(base_wfs, "sciensano:wastewatertreatmentplantspmmv"))

# Helper: read first CSV inside a ZIP from URL
read_csv_from_zip_url <- function(zip_url) {
  ...
  if (length(csvs) == 0) stop("No CSV found inside zip: ", zip_url)
  
  read.csv(csvs[1])
}

# FLU data (ZIP download)
df_flu <- read_csv_from_zip_url(
  "https://ac.ngi.be/remoteclient-open/GeoBePartners-open/Sciensano/a27da9c0-6c85-11f0-9e86-00be433d8c24_csv.zip"
)

# RSV data (ZIP download)  (link iz metapodataka)
df_rsv <- read_csv_from_zip_url(
  "https://ac.ngi.be/remoteclient-open/GeoBePartners-open/Sciensano/e6681f9f-6c76-11f0-bb07-00be433d8c24_csv.zip"
)

# join all
df <- dplyr::bind_rows(df_sc, df_pmmv, df_flu, df_rsv)
